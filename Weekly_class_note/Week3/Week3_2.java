...
public class Week3_2 {
    ...
    public static void selectionSort(int[] myNumbers) {
         // outer loop
        for(int sortpos = 0; sortpos < myNumbers.length - 1; sortpos++){
            int minipos = myNumbers[sortpos];
            
            // find the minimum value's index
            for(int j = sortpos+1; j <myNumbers.length; j++){
               if(myNumbers[j] < myNumbers[minipos]){
                  minipos = j;
               }
            }
            // swap values at the minimum indexand the sorting index
            int temp = myNumbers[minipos];
            myNumbers[minipos] = myNumbers[sortpos];
            myNumbers[sortpos] = temp;
        }
    }
